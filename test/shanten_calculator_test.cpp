...
TEST(shanten, many_cases) {
    ...
    for (const std::string& testcase : {
        ...
        std::string(TEST_RESOURCES_DIR) + "/shanten_testcases/p_normal_10000.txt",
        std::string(TEST_RESOURCES_DIR) + "/shanten_testcases/p_tin_10000.txt"
    }) {
        clock_t start = clock();
        ...
        while (std::getline(ifs, line)) {
            ...
            EXPECT_EQ(ShantenCalculator::ShantenThirteenOrphans(tiles), thirteen_orphans);
