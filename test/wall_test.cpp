...
TEST(wall, initial_hand) {
    ...
    EXPECT_EQ(wall.initial_hand_tiles(AbsolutePos::kInitEast), Tile::Create({48, 16, 19, 34, 2, 76, 13, 7, 128, 1, 39, 121, 87}));
    EXPECT_EQ(wall.initial_hand_tiles(AbsolutePos::kInitSouth), Tile::Create({17, 62, 79, 52, 56, 57, 82, 98, 32, 103, 24, 70, 54}));
    EXPECT_EQ(wall.initial_hand_tiles(AbsolutePos::kInitWest), Tile::Create({55, 30, 12, 26, 31, 90, 3, 4, 80, 125, 66, 102, 78}));
    EXPECT_EQ(wall.initial_hand_tiles(AbsolutePos::kInitNorth), Tile::Create({120, 130, 42, 67, 114, 93, 5, 61, 20, 108, 41, 100, 84}));
    ...
    wall = Wall(5, tiles);
    EXPECT_EQ(wall.initial_hand_tiles(AbsolutePos::kInitEast), Tile::Create({52, 41, 105, 21, 83, 40, 17, 89, 50, 4, 11, 60, 120}));
    EXPECT_EQ(wall.initial_hand_tiles(AbsolutePos::kInitSouth), Tile::Create({117, 42, 114, 28, 80, 87, 73, 2, 31, 27, 68, 25, 47}));
    EXPECT_EQ(wall.initial_hand_tiles(AbsolutePos::kInitWest), Tile::Create({70, 124, 97, 56, 33, 71, 13, 118, 24, 86, 90, 101, 34}));
    EXPECT_EQ(wall.initial_hand_tiles(AbsolutePos::kInitNorth), Tile::Create({5, 32, 81, 46, 7, 119, 129, 116, 104, 103, 30, 130, 3}));
    ...
    wall = Wall(7, tiles);
    EXPECT_EQ(wall.initial_hand_tiles(AbsolutePos::kInitEast), Tile::Create({19, 55, 77, 86, 131, 51, 135, 33, 119, 11, 64, 97, 110}));
    EXPECT_EQ(wall.initial_hand_tiles(AbsolutePos::kInitSouth), Tile::Create({12, 31, 25, 22, 38, 26, 8, 1, 122, 94, 52, 39, 74}));
    EXPECT_EQ(wall.initial_hand_tiles(AbsolutePos::kInitWest), Tile::Create({40, 7, 81, 21, 111, 130, 62, 18, 98, 93, 71, 103, 82}));
    EXPECT_EQ(wall.initial_hand_tiles(AbsolutePos::kInitNorth), Tile::Create({61, 132, 36, 46, 129, 89, 14, 59, 101, 16, 127, 123, 47}));
}
...
TEST(wall, WallEqualityOverDevices) {
    ...
    ASSERT_EQ(tiles.at(0), Tile(49));
    ASSERT_EQ(tiles.at(1), Tile(128));
    ASSERT_EQ(tiles.at(2), Tile(6));
    ASSERT_EQ(tiles.at(3), Tile(50));
    ASSERT_EQ(tiles.at(4), Tile(62));
}
