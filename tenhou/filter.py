...
class Filter:
    ...
    def has_ng_username_char(self, bad_chars: List[str]) -> bool:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="""Filter Tenhou's mjlog.
    ...
    parser.add_argument('--ng-username-chars', nargs='+', help='List of chars unappropriate as username')
    ...
    for mjlog in os.listdir(args.mjlog_dir):
        ...
        if (not f.has_valid_seed()) or \
           ...
           (args.rm_users and f.has_ng_username_char(args.ng_username_chars)):
            rm(path_to_mjlog)
            removed_cnt += 1
        total_cnt += 1
    print(f"Done. # of removed file = {removed_cnt}/{total_cnt} = {100.0 * removed_cnt / total_cnt:.02f}%")
