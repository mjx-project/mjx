...
std::string SwapTiles(const std::string &json_str, Tile a, Tile b) {
  ...
  // event history
  for (int i = 0; i < state.public_observation().event_history().events_size(); ++i) {
    auto mevent = state.mutable_public_observation()->mutable_event_history()->mutable_events(i);
    if (Any(mevent->type(),
...
std::string TruncateAfterFirstDraw(const std::string &json) {
  ...
  Assert(status.ok());
  auto events = state.mutable_public_observation()->mutable_event_history()->mutable_events();
