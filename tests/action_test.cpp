...
TEST(action, Encode) {
  // Discard
  EXPECT_EQ(Action::Encode(Action::CreateDiscard(AbsolutePos::kInitEast, Tile(17))), 4);
  // Discard red
  EXPECT_EQ(Action::Encode(Action::CreateDiscard(AbsolutePos::kInitEast, Tile(16))), 34);
  // Chi
  ...
  EXPECT_EQ(Action::Encode(Action::CreateOpen(AbsolutePos::kInitEast,
                                              Chi::Create(tiles, Tile(12)))), 59);
  // Pon
  EXPECT_EQ(Action::Encode(Action::CreateOpen(AbsolutePos::kInitEast,
                                              Pon::Create(Tile(17), Tile(16), RelativePos::kLeft))), 71);
  // Pon w/ red
  EXPECT_EQ(Action::Encode(Action::CreateOpen(AbsolutePos::kInitEast,
                                              Pon::Create(Tile(16), Tile(17), RelativePos::kLeft))), 101);
  // Kan
  EXPECT_EQ(Action::Encode(Action::CreateOpen(AbsolutePos::kInitEast,
  ...
  // Kyusyu
  EXPECT_EQ(Action::Encode(Action::CreateNineTiles(AbsolutePos::kInitEast)), 141);
  // No
