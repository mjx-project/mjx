...
def reproduce_wall_from_mjlog(mjlog_str: str, use_cache=True) -> List[Tuple[List[int], List[int]]]:
    """ SeedをXMLから切り出す
    ...
    >>> wall_dices = reproduce_wall_from_mjlog(mjlog, True)
    >>> len(wall_dices)
    100
    """
    root = ET.fromstring(mjlog_str)
...
def reproduce_wall_from_seed(seed: str, use_cache=True) -> List[Tuple[List[int], List[int]]]:
    ...
    out: List[str]
    if use_cache and os.path.exists(seed_cache):
