...
class Open {
 public:
  ...
  static int From(std::uint16_t bits);  // In added kan, it's the opponent player from whom
  ...
  static int StolenTile(std::uint16_t bits);  // kan_added => poned tile by others, kan_closed =>
                                              // tile id represented at left 8 bits
  static int LastTile(std::uint16_t bits);    // Last tile added to this open tile sets. kan_added
