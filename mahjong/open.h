...
namespace mj
{
    ...
    public:
        ...
        RelativePos From() const;  // In added kan, it's the opponent player from whom the pon was declared (not kan)
        Tile At(std::size_t i) const;  // sorted by tile id
        std::size_t Size() const;
        std::vector<Tile> Tiles() const;  // sorted by tile id
        std::vector<Tile> TilesFromHand() const;  // sorted by tile id. chi => 2 tiles, pon => 2, kan_opened => 3, kan_closed => 4, kan_added => 2
        Tile StolenTile() const; // kan_added => poned tile by others, kan_closed => tile id represented at left 8 bits
        Tile LastTile() const;  // Last tile added to this open tile sets. kan_added => lastly kaned tile, the others => stolen()
    ...
    public:
        KanClosed() = delete;
        static Open Create(Tile tile);  // TODO: check which tile id does Tenhou use? 0? drawn tile? This should be aligned.
