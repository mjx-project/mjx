...
namespace mjx
{
    AgentBatchGrpcServerImpl::AgentBatchGrpcServerImpl(std::unique_ptr<Strategy> strategy, int batch_size, int wait_ms) :
    ...
    grpc::Status
    AgentBatchGrpcServerImpl::TakeAction(grpc::ServerContext *context, const mjxproto::Observation *request, mjxproto::Action *reply) {
        // Observationデータ追加
    ...
    void
    AgentBatchGrpcServer::RunServer(std::unique_ptr<Strategy> strategy, const std::string &socket_address, int batch_size,
                               int wait_ms) {
        std::unique_ptr<grpc::Service> agent_impl = std::make_unique<AgentBatchGrpcServerImpl>(std::move(strategy), batch_size, wait_ms);
