...
namespace mjx {
Env::Env() : agents_(std::vector<internal::PlayerId>(4)) {
  for (int i = 0; i < 4; ++i) {
...
Env::step(std::unordered_map<internal::PlayerId, std::string>&& act_dict) {
  actions_.clear();
  ...
  if (state_.IsRoundOver()) {
    if (state_.IsGameOver()) {
      for (const auto& agent : agents_) {
        assert(state_.result().rankings[agent] >= 1 &&
               state_.result().rankings[agent] <= 4);
...
std::unordered_map<internal::PlayerId, std::string> Env::ObservationsJson() {
  ...
  for (const auto& [agent, obs] : observations_) {
    std::string json;
    google::protobuf::util::MessageToJsonString(obs.proto(), &json);
