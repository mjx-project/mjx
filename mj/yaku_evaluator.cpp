...
namespace mj
{
    ...
    WinScore YakuEvaluator::Eval(const WinInfo& win_info) noexcept {
        BOOST_ASSERT(win_cache().Has(win_info.hand.closed_tile_types));
    ...
    int YakuEvaluator::CalculateFu(
        ...
        BOOST_ASSERT(win_info.hand.win_tile);
        ...
        BOOST_ASSERT(!(win_info.hand.is_menzen && win_info.hand.stage == HandStage::kAfterRon && !yakus.count(Yaku::kPinfu) && fu < 40));
        return fu;
    }
